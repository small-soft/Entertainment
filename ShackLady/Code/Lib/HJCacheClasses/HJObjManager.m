...
static HJObjManager  *sharedManager = nil;
...
+ (void)cleanCacheInBackground{
    ...
    if (objManager.fileCache) {
//        objManager.fileCache.maintenanceThread = [[[NSThread alloc] initWithTarget:objManager.fileCache selector:@selector(emptyCache) object:nil] autorelease];
//        [objManager.fileCache.maintenanceThread start];
        [objManager.fileCache emptyCache];
    }

}
+ (HJObjManager *)sharedManager {
    if (nil==sharedManager) {
        sharedManager = [[HJObjManager alloc] init];
    }
	return sharedManager;
}

+ (void)setSharedManager:(HJObjManager *)manager {
	[manager retain];
